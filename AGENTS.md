...
- **Tags de Release**: Sempre que uma release for realizada, uma tag Git (`vX.X.X` seguindo o Semantic Versioning) deve ser gerada e atualizada no projeto, mantendo a convenção estabelecida em outros projetos base (como sition-web e fitness-web). Ao criar documentação de changelog ou release notes, lembre-se de associar a essas tags.
